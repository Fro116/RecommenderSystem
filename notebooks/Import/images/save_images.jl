...
function import_data(folder)
    qrun(
        `rclone --retries 10 copyto r2:rsys/database/import/$folder.tar $datadir/$folder.tar`,
    )
    if !ispath("$datadir/$folder.tar")
        mkpath("$datadir/$folder")
        return
    end
    cmds = ["cd $datadir", "tar xf $folder.tar", "rm $folder.tar"]
    cmd = join(cmds, " && ")
    qrun(`sh -c $cmd`)
end
...
function get_diffs()
    ...
    for x in readdir("$datadir/srimages")
        name, _, _ = split(x, ".")
...
function upload_images(to_add, to_delete)
    ...
    @showprogress for fn in to_add
        name, ext = split(fn, ".")
        for x in ["medium", "large"]
            src = "$datadir/srimages/$name.$x.webp"
            dst = "r2:cdn/images/cards/$name.$x.webp"
            qrun(`rclone --retries=10 copyto $src $dst`)
        end
    end
