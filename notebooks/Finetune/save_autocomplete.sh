...
gcloud sql import csv inference $bucket/$df.csv --database=postgres --table=${db}_staging -q
gcloud storage rm $bucket/$df.csv
psql "$connstr" -c "CREATE INDEX ${db}_staging_source_prefix_idx ON ${db}_staging (source, prefix);"
