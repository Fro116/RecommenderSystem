...
(uvicorn layer1:app --host 0.0.0.0 --port 4101 --log-level warning |& julia logrotate.jl $logs/layer1.1.log) &
(uvicorn layer1:app --host 0.0.0.0 --port 4102 --log-level warning |& julia logrotate.jl $logs/layer1.2.log) &
(uvicorn layer1:app --host 0.0.0.0 --port 4103 --log-level warning |& julia logrotate.jl $logs/layer1.3.log) &
(uvicorn layer1:app --host 0.0.0.0 --port 4104 --log-level warning |& julia logrotate.jl $logs/layer1.4.log) &
(julia -t auto layer2.jl 4002 1 "http://localhost:4101/proxy,http://localhost:4102/proxy,http://localhost:4103/proxy,http://localhost:4104/proxy" true 10 "../../environment" |& julia logrotate.jl $logs/layer2.log) &
...
(julia backup.jl |& julia logrotate.jl $logs/backup.log) &
