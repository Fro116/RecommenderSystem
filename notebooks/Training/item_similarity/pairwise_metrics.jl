...
function get_embedding_matrix(medium)
    registry = Dict()
    HDF5.h5open("$datadir/item_similarity/output.embeddings.$medium.h5", "r") do f
        for k in keys(f)
...
function get_adaptations(split, medium)
    df = CSV.read("$datadir/item_similarity/adaptations.$split.csv", DataFrames.DataFrame)
...
function recall_at_k(df::DataFrames.DataFrame, M::Matrix{<:Real}, k::Int)
    ...
    @showprogress Threads.@threads for i = 1:length(unique_sources)
        ...
        ground_truth_counts =
            Dict(row.target => row.relevance for row in eachrow(source_rows))
        count_in_top_k =
...
function save_metrics()
    ...
    for medium in [0, 1]
        testmask = HDF5.h5open("$datadir/item_similarity/pairs.$medium.h5", "r") do file
            read(file["testmask"])
        end
        df = CSV.read("$datadir/item_similarity/pairs.$medium.csv", DataFrames.DataFrame)
        filter!(x -> x.cliptype == "medium$medium" && x.score != 0, df)
        ...
        filter!(x -> testmask[x.source, x.target] != 0, df)
    ...
    JLD2.save("$datadir/pairwise.embeddings.jld2", d)
    metrics = make_metric_dataframe(ret)
    display(metrics)
    CSV.write("$datadir/pairwise.embeddings.csv", metrics)
end
