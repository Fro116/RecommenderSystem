...
from megatron.core import parallel_state as pstate
from torch.nn.parallel import DistributedDataParallel as DDP
from torch.utils.data import DataLoader, IterableDataset
from tqdm import tqdm
from transformer_engine.pytorch import fp8_autocast
from transformer_impl import TransformerStack, TransformerStackConfig
from transformer_model import TransformerModel

warnings.filterwarnings("ignore", ".*Initializing zero-element tensors is a no-op.*")
warnings.filterwarnings("ignore", ".*Sparse CSR tensor support is in beta state.*")
warnings.filterwarnings("ignore", r".*Dynamo detected a call to a `functools\.lru_cache`.*")
warnings.filterwarnings("ignore", ".*Dynamo does not know how to trace the builtin.*")
...
parser.add_argument("--gputype", type=str)
    ...
    for data in dataloader:
        with torch.no_grad():
            with torch.amp.autocast(f"cuda:{local_rank}", dtype=torch.bfloat16):
                sf = model.module.transformers
                with fp8_autocast(enabled=sf.cfg.use_fp8, fp8_recipe=sf._fp8_recipe):
...
def checkpoint_model(
    ...
    if global_rank == 0 and args.finetune is None:
...
def upload(global_rank, logger):
    if global_rank != 0 or args.finetune is not None or args.debug:
        return
...
def init_dist():
    ...
    # If launched with torchrun, env vars are set. Otherwise default to single-GPU.
