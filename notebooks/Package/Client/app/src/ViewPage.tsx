...
const loadingMessages = [
  ...
  'Consulting with the head maid...',
  'Polishing your recommendations...',
  'The maids are working hard...',
...
const ViewPage: React.FC<ViewPageProps> = ({ isMobile }) => {
  ...
  const [loadingBgImage, setLoadingBgImage] = useState<string>('');
  ...
  useEffect(() => {
    ...
    const backgroundImages = Math.random() < 0.9 ? backgroundImagesL : backgroundImagesN;
    const randomBgIndex = Math.floor(Math.random() * backgroundImages.length);
    setLoadingBgImage(backgroundImages[randomBgIndex]);
  ...
  if (isLoading && results.length === 0) {
    return (
      <div
        ...
        style={{
          backgroundImage: loadingBgImage ? `url('${loadingBgImage}')` : 'none',
        }}
      >
