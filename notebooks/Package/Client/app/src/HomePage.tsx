...
const HomePage: React.FC = () => {
  ...
  const [autocompleteResults, setAutocompleteResults] = useState<
    AutocompleteItem[]
  >([]);
  const [autocompleteDisabled, setAutocompleteDisabled] =
  ...
  const handleAutocompleteClick = (item: AutocompleteItem) => {
    ...
    if (queryMode === "user") {
      ...
      if (source && itemid) {
        ...
        console.error(
          "Autocomplete item is missing 'source' or 'itemid'",
          item,
        );
  ...
  useEffect(() => {
    const handleOutside = (evt: MouseEvent) => {
      if (
        ...
        !searchContainerRef.current.contains(evt.target as Node)
      ) {
        setAutocompleteResults([]);
      }
    };
    ...
    <>
              ...
              <div className="autocomplete-container">
                {suggestionsToShow.map((item, idx) => {
                  ...
                  if (queryMode === "user") {
                    ...
                    const genderAge = [item.gender, item.age]
                      .filter(Boolean)
                      .join(", ");
                    if (genderAge) {
                    ...
                    >
                      {queryMode === "user"
                        ? (item.avatar || item.missing_avatar) && (
                            <img
                              className="autocomplete-avatar-user"
                        ...
                        : item.image?.[0]?.url && (
                            <img
                              className="autocomplete-avatar-item"
                      ...
                      {queryMode === "user" ? (
                        <div className="autocomplete-item-text-wrapper">
                          ...
                          <div className="autocomplete-metadata">
                            {userMetadata}
                          </div>
                        </div>
                      ) : (
                        <div className="autocomplete-item-text-wrapper">
                          ...
                          <div
                            className="autocomplete-matched"
                            data-metadata={itemMetadata}
                          >
          ...
          >
            ...
            >
              ...
              <span className="query-mode-button-description">
                {queryMode === "user"
                  ? "Get recommendations based on a user's profile."
                  : "Discover works similar to a specific anime or manga."}
              </span>
            ...
            {showModeDropdown && (
              <ul className="query-mode-list">
                ...
                >
                  ...
                  <div className="query-mode-description">
                    Get recommendations based on a user's profile.
                  </div>
                </li>
                <li
              ...
              {queryMode === "user"
                ? (
                    ...
                    <button
                      key={src}
                      type="button"
                      className={`source-button ${activeSource === src ? "selected" : ""}`}
                ...
                : (["anime", "manga"] as ItemType[]).map((type) => {
                    const buttonText =
                      type === "anime" ? "an anime" : "a manga";
                    return (
                      <button
                        key={type}
                        type="button"
                        className={`source-button ${itemType === type ? "selected" : ""}`}
      ...
      {errorMessage && (
        <div
          ...
          style={{ margin: "40px auto 0", width: "80%", maxWidth: "600px" }}
        >
