...
function compute_alpha(username, source, medium, version, modelport)
    cmdstring = (
        "curl http://localhost:$modelport/query?username=$username&source=$source&medium=$medium"
    )
    cmd = Cmd(String.(split(cmdstring)))
    run(pipeline(cmd; stdout = devnull, stderr = devnull))
    seen =
        get_raw_split("rec_training", medium, [:itemid], nothing, username, source).itemid
    ptw =
        get_split(
            "rec_training",
            "plantowatch",
...
Oxygen.@get "/query" function (req::HTTP.Request)
    ...
    for medium in ALL_MEDIUMS
        compute_alpha(username, source, medium, version, modelport)
    end
end
